...
static uint32_t decode_planes(Player *player, FILE *input, uint16_t kind, uint32_t size)
{
    ...
    if (!frame || fread(frame, 1, size, input) != size)
        exit(2);
    ...
    switch (kind) {
    case I_FRAME:
        HVQM4DecodeIpic(&player->seqobj, frame + 4, player->present);
        break;
    case P_FRAME:
        HVQM4DecodePpic(&player->seqobj, frame + 4, player->present, player->past);
        break;
    case B_FRAME:
        HVQM4DecodeBpic(&player->seqobj, frame + 4, player->present, player->past, player->future);
        break;
    default:
        exit(2);
    }
    ...
    for (uint32_t block = 0; block < header.blocks; ++block) {
        ...
        for (uint32_t n = 0; n < videos + audios; ++n) {
            ...
            for (unsigned p = 0; p < 3; ++p)
                length += state->planes[p].size_in_samples;
            FILE *out = stdout;
            if (stream) {
            ...
            if (!out || fwrite(pixels, 1, length, out) != length)
                return 2;
            if (!stream && fclose(out))
                return 2;
        }
        base += videos;
    }
